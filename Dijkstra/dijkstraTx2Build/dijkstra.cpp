...
int main(int argc, char* argv[]) {
	...
	vector<vector<int>> H(rolSize, vector<int>(colSize, INT_MAX));
	...
	*/
	...
	H[startPoint.first][startPoint.second] = 0;
	...
	while (!que.empty()) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (H[cur.first][cur.second] < H[tmpRol][tmpCol]) {
				H[tmpRol][tmpCol] = H[cur.first][cur.second] + 1;
